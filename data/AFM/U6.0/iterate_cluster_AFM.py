...
cix_name = 'hubbard_U_'+str(Uc)+'_normal_AFM.cix'
...
def AssembleHk(ikx,iky,c,s):
    ...
    """
    Hk = zeros((8,8), dtype=complex)    
    Hk[0,0] =  -(2.0 + c[ikx] + c[iky])
    Hk[0,2] = s[ikx]*1j
    Hk[0,4] = s[iky]*1j
    Hk[1,1] =  -(2.0 + c[ikx] + c[iky])
    Hk[1,3] = s[ikx]*1j
    Hk[1,5] = s[iky]*1j
    Hk[2,2] = c[ikx]-c[iky]
    Hk[2,6] = s[iky]*1j
    ...
    Hk[5,5] = -(c[ikx]-c[iky])
    Hk[5,7] = s[ikx]*1j
    Hk[6,6] = 2.0 + c[ikx] + c[iky]
    Hk[7,7] = 2.0 + c[ikx] + c[iky]
    for i in range(8):
        for j in range(i):
...
def ComputeGloc(klist,w,Sg,mu,c,s):
    ...
    """
    Gloc = zeros((len(w),8,8), dtype=complex)
    Id = identity(8)
...
def ComputeDelta(w,Sg,Eimp):
    ...
    """
    Delta = zeros( (len(w),8,8), dtype=complex)
    Id = identity(8)
    for iw in range(len(w)):
...
if __name__ == '__main__':
    ...
    EimpCF = [-2,-2,0,0,0,0,2,2]   # crystal fields, given by (\sum_k H_k) of the model
    ...
    if not glob.glob('Sig.out'): # does not exist
        ...
        ss = vstack( ( zeros(len(om)), -ones(len(om))*gamma, zeros(len(om)), -ones(len(om))*gamma, zeros(len(om)), -ones(len(om))*gamma, zeros(len(om)), -ones(len(om))*gamma, zeros(len(om)), -ones(len(om))*gamma, zeros(len(om)), -ones(len(om))*gamma ) )
        data = vstack( (om, ss) )
    else:
    ...
    for it in range(Niter):
        ...
        om = data[0]
        S00u = data[1]+data[2]*1j
	S00d = data[3]+data[4]*1j
        Sp0u = data[5]+data[6]*1j
        Sp0d = data[7]+data[8]*1j
        Sppu = data[8]+data[9]*1j
	Sppd = data[10]+data[11]*1j
        
        # create log-mesh in frequency
        ind_om = create_log_mesh(om,nom,ntail)
        w = zeros(len(ind_om))
        Sg = zeros( (len(ind_om),8,8), dtype=complex)
        for i,iw in enumerate(ind_om):
            w[i] = om[iw]
            # self-energy for plaquette on logarithmic mesh
            Sg[i] = [ [S00u[iw],0,0,0,0,0,0,0],[0,S00d[iw],0,0,0,0,0,0],[0,0,Sp0u[iw],0,0,0,0,0],[0,0,0,Sp0d[iw],0,0,0,0],[0,0,0,0,Sp0u[iw],0,0,0],[0,0,0,0,0,Sp0d[iw],0,0],[0,0,0,0,0,0,Sppu[iw],0],[0,0,0,0,0,0,0,Sppd[iw]] ]
        ...
        D00u = ReconstructOnEntireMesh(Delta[:,0,0], om,nom,w)
	D00d = ReconstructOnEntireMesh(Delta[:,1,1], om,nom,w)
        Dp0u = ReconstructOnEntireMesh(0.5*(Delta[:,2,2]+Delta[:,4,4]), om,nom,w)
        Dp0d = ReconstructOnEntireMesh(0.5*(Delta[:,3,3]+Delta[:,5,5]), om,nom,w)
        Dppu = ReconstructOnEntireMesh(Delta[:,6,6], om,nom,w)
	Dppd = ReconstructOnEntireMesh(Delta[:,7,7], om,nom,w)
        
        datao = vstack( (om,D00u.real,D00u.imag,D00d.real,D00d.imag,Dp0u.real,Dp0u.imag,Dp0d.real,Dp0d.imag,Dppu.real,Dppu.imag,Dppd.real,Dppd.imag) )
